...
If you want to split the results of the alignment by chromosome, use `mpiBWAByChr` that produces

  the results SAM file will contain the header, and for each chromosome of the header mpiBWA will yield a SAM file plus a discordant and a unmapped SAM files.
...
* Individual chrN.sam files with aligned reads on each chrN (ChrN are the chromosome name from the header). The chrN.sam contains the header for the chrN but the positions are not sorted. They can be sorted independantly with [mpiSORT](https://github.com/bioinfo-pf-curie/mpiSORT).
...
This software needs a parallel filesystem for execution. The programm has been tested with  [Lustre](http://lustre.org/) and [BeeGFS](https://www.beegfs.io/).
